...
class income_categories(m.Model):
    ...
    title = m.CharField(verbose_name="Title")
    color = m.CharField(verbose_name="Color of income's bg")

class expense_categories(m.Model):
    user_id = m.ForeignKey(User, on_delete=m.CASCADE)
    title = m.CharField(verbose_name="Title")
    color = m.CharField(verbose_name="Color of expense's bg")
...
class one_time_incomes(m.Model):
    ...
    user_id = m.ForeignKey(User, on_delete=m.CASCADE)
    reason = m.CharField(verbose_name="Reason")
...
class subscription_periods(m.Model):
    name = m.CharField(verbose_name="Name")
    written_value = m.CharField(verbose_name="Wrriten value")
